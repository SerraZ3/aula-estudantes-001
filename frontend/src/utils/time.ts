...
const time = {
  ...
  date2String(date: string | Date, withTime?: boolean, dateSymbol: string = '/'): string {
    ...
    if (withTime)
      return `${day}${dateSymbol}${month}${dateSymbol}${year} ${hour}:${minutes}:${seconds}`;
    return `${day}${dateSymbol}${month}${dateSymbol}${year}`;
  },
  date2StringFormatDatabase(date: string | Date, withTime?: boolean, dateSymbol?: string): string {
    const newDate = this.date(date);
    let day = `${newDate.getDate()}`;
    let month = `${newDate.getMonth() + 1}`;
    const year = `${newDate.getFullYear()}`;
    ...
    seconds = this.checkIfExistZero(seconds);
    if (withTime)
      return `${year}${dateSymbol}${month}${dateSymbol}${day} ${hour}:${minutes}:${seconds}`;
